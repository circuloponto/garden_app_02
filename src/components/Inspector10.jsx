    ...
    if (typeof hoveredElectron === 'object' && hoveredElectron.type) {
      if (hoveredElectron.type === 'connection') {
        ...
        if (fromChordType && toChordType) {
          ...
          // Calculate electron notes directly - always recalculate for each electron hover
          // This ensures each electron gets its own unique notes based on its specific offset
          const electronNotes = result.scale
          ...
          // Store these notes for display
          setDisplayedElectronNotes(electronNotes);
          
          console.log(`Inspector2: Calculated electron notes for ${hoveredElectron.electronClass} with offset ${offsetValue}:`, electronNotes);
      ...
      if (connectionMatch) {
        ...
        if (fromChordType && toChordType) {
          ...
          console.log(`Inspector2: Calculated electron notes for legacy electron with offset ${offsetValue}:`, electronNotes);
